...
import { CastEvent, CastMediaInfo, CastMediaStatus, CastMetadata, CastTextTrack, LoadMediaOptions, LoadQueueOptions, MetadataType, PlayerState, QueueInsertItemOptions, QueueInsertItemsOptions, QueueItem, QueueItemOptions, QueueType, QueueUpdateItemsOptions, RepeatMode, StreamType } from './types';
...
export class CastButton extends CastButtonBase {
  ...
  buildMediaInfo(mediaInfo: CastMediaInfo) {
    ...
    // Build media info
    const mediaInformation = new MediaInfo.Builder(mediaInfo.contentId).setContentType(mediaInfo.contentType).setStreamType(MediaInfo[streamType]);
    ...
    if (mediaInfo.textTracks && mediaInfo.textTracks.length > 0) {
      ...
      mediaInfo.textTracks.forEach((item, index) => {
        const track = new MediaTrack.Builder(index + 1, MediaTrack.TYPE_TEXT).setContentId(item.src).setContentType(item.contentType).setSubtype(MediaTrack.SUBTYPE_SUBTITLES).setName(item.name).setLanguage(item.language).build();
        tracks.add(track);
      });
  ...
  loadMedia(media: CastMediaInfo, options?: LoadMediaOptions) {
    ...
    const requestData = new MediaLoadRequestData.Builder().setAutoplay(new java.lang.Boolean(options.autoplay)).setCredentials(options.credentials).setCredentialsType(options.credentialsType).setCurrentTime(options.startTime).setCustomData(options.customData).setMediaInfo(builtMediaInfo).setPlaybackRate(options.playbackRate).setQueueData(null);
