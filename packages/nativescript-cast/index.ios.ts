...
import { CastMediaInfo, CastMetadata, CastTextTrack, CastTextTrackStyle, LoadMediaOptions, LoadQueueOptions, MetadataType, QueueInsertItemOptions, QueueInsertItemsOptions, QueueItem, QueueItemOptions, QueueType, QueueUpdateItemsOptions, RepeatMode, ResumeState, StreamType } from './types';
  ...
  buildMediaInfo(mediaInfo: CastMediaInfo) {
    ...
    if (mediaInfo.textTracks && mediaInfo.textTracks.length > 0) {
      ...
      mediaInfo.textTracks.forEach((track, index) => {
        mediaTracks.addObject(GCKMediaTrack.alloc().initWithIdentifierContentIdentifierContentTypeTypeTextSubtypeNameLanguageCodeCustomData(index + 1, track.src, track.contentType, GCKMediaTrackType.Text, GCKMediaTextTrackSubtype.Subtitles, track.name, track.language, null));
      });
    }
