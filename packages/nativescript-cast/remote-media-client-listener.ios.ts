...
import { CastButton, convertMediaInfo, queueTypeEnumToString, repeatModeEnumToString } from './index.ios';
import { CastEvent, PlayerState, CastMediaStatus } from './types';
...
class RemoteMediaClientListenerImpl extends NSObject implements GCKRemoteMediaClientListener {
  ...
  protected toCastMediaStatus(mediaStatus: GCKMediaStatus): CastMediaStatus {
    ...
    const activeTrackIds = mediaStatus.activeTrackIDs ? Utils.ios.collections.nsArrayToJSArray(mediaStatus.activeTrackIDs).map((trackId) => +trackId) : [];
